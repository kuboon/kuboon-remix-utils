...
These were `@kuboon/remix-*` until the `@remix-kbn` scope existed. The `remix-` prefix went with the move, since the scope already says it — `@kuboon/remix-ssg` is `@remix-kbn/ssg`. Each package's CHANGELOG records where its old series stopped; JSR cannot unpublish, so nothing under the old names has gone away.
...
| [`ui-pinch-pan`](./packages/ui-pinch-pan)                       | [`@remix-kbn/ui-pinch-pan`](https://jsr.io/@remix-kbn/ui-pinch-pan)                       | Two-finger pinch and pan as a `@remix-run/ui` mixin                                                  |

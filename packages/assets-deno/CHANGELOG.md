...
- Resolution, loading, and TypeScript/JSX transpilation are delegated to `@deno/loader`, so everything runs in-process: no `deno` subprocess and no `--allow-run`. Only the runtime graph is served; type-only modules are excluded.
- Lower-level pieces are exported for custom pipelines: `loadModuleGraph`, `PathRegistry` / `candidatePathFor`, and `rewriteImports`.
